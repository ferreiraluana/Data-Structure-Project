...
heap* create_heap()
{
  ...
  new_heap->size = 0;
  int i;
  for(i=1;i<=256;i++) new_heap->data[i] = NULL;
}
...
void max_heapify(heap *hp,int i )// i = 1
{
  ...
  if(left_index <= hp->size && hp->data[left_index]->frequence > hp->data[i]->frequence) biggest =  left_index;
  else { biggest = i; }
...
void build_max_heap(heap *hp)
{
  int i;
  int size = hp->size;
  for(i=size/2;i>=1;i--) max_heapify(hp,i);
}
...
btree *building_huff_tree(heap *hp, int t)
{
  ...
  btree *sum = create_btree('*', left->frequence + right->frequence, left, right);
...
{
    for(int i = 0;i<=256;i++) trees[i] = NULL;
}
