...
btree* create_huffman_tree(unsigned *freq)
{
  ...
  btree *huff= NULL;
    ...
    for(i = 0;i < 256;i++) // adicionando as frequencias na heap /// i era === 1
    {
      if(freq[i] != 0)
      {
...
void Hash(hash *hs, btree *arv, byte *binary, long long int *i)
{
  if(arv != NULL)
  {
    ...
    {
      if(arv->c == NULL) // testar com '\0' ou NULL
      {
        element *new_element = (element*) malloc(sizeof(element));
        strcpy(new_element->binary , binary);
...
btree *tree_from_pre_order(btree *huff, byte *pre_order, int *cont, int size)
{
  if(cont[0] == 0)
  {
    huff = create_btree(pre_order[cont[0]], 0, NULL, NULL);
    cont[0]++;
    huff->left = tree_from_pre_order(huff->left, pre_order, cont, size);
    huff->right = tree_from_pre_order(huff->right, pre_order, cont, size);
  }
  else
  {
    if(pre_order[cont[0]] == '\\')
...
{
  FILE *output = fopen("descompressed.jpg","wb");
  btree *aux = huff;
  int i,cont = 0;
  byte c;
...
int main()
{
  ...
  if(!strcasecmp(caminho,"c"))
  {
    ...
    output = construct_file(input, dictionary, pre_order, header, tam, c); // gera um output.txt (logo serah .huff) na sua pasta
    free(huff);
    free(dictionary);
    fclose(output);
